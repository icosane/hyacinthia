...
ffmpeg_dir = os.path.join(base_dir, "models", "ffmpeg", "bin")
vocoder_dir = os.path.join(base_dir, "models", "vocoder")

def get_lib_paths():
    if getattr(sys, 'frozen', False):  # Running inside PyInstaller
        base_pkg = os.path.join(sys.prefix)
    else:  # Running inside a virtual environment
        base_pkg = os.path.join(sys.prefix, "Lib", "site-packages")

    nvidia_base_libs = os.path.join(base_pkg, "nvidia")
    ...
    if sys.platform == "win32":
        ...
        torch = os.path.join(base_pkg, "torch")
    else:
    ...
    return [cuda_runtime, cublas, cudnn, torch]
